...
Selecting two rows shows the action bar with the count and the summed size; "Move to Trash" opens the dialog with a preview of exactly those paths; "Delete permanently" opens the same dialog already in Permanent mode, with its confirm button still disabled until the acknowledgement; confirming removes the rows from the table and clears the selection; the header total shrinks; cancelling leaves everything alone; a batch where one entry is blocked still deletes the other and the result view names the blocked one; switching directories clears the selection; a rescan (new generation) clears the selection.

**Not the breadcrumbs.** An earlier draft asked for "the ancestors' sizes in the breadcrumbs" to shrink. A `Crumb` is `{ id, name }` and `Breadcrumbs` renders names, so there is no size there to watch. What the user can see shrink is the header summary — which is the same thing Task 16's e2e asserts, and it arrives through the refresh below.
...
**The grid question, decided here: phase 2a does not promote the table.** `role="grid"` is not a label, it is a contract — one tab stop with a roving `tabIndex`, and arrow keys that walk cells — and this table gives every one of its 500 rows `tabIndex={0}`. Claiming the role without the tab model tells a screen reader to switch to application navigation for a widget that does not answer it, which is worse than the implicit `table` it is today, and the change would move the e2e selectors a second time and rewrite the focus tests that pin the current behaviour. So the promotion is a follow-up of its own (with the roving tab stop, which this table wants anyway), and the selection is announced another way in the meantime: the action bar's count sits in a `role="status"` region that is mounted whether or not anything is ticked — a region added to the page together with its first words is announced by no screen reader reliably — and the visible copy of the same words is `aria-hidden`, so it is heard once. Task 16 therefore inherits one selector move (the checkbox column), not two.
...
**And the id this page navigates by is one of them — the fourth door.** Invalidating fixes the cache and not the key: the refetch goes out with `current.id`, which was resolved against the arena the splice replaced. It is not only the deleted rows that move. `install_patches` says it outright ("every id in the arena is a new id now"), and `replace_subtrees` re-sorts every sibling group holding a node whose size changed — which is every ancestor of the deletion, the directory on screen included. Delete 40 GB out of a folder, watch it drop below its siblings, and the id it used to have now belongs to one of them: the refetch comes back with another directory's rows and another directory's breadcrumbs, and nothing says so. So a batch that patched anything also sends the Explorer back to `ROOT_ID`, the one id a splice cannot move. A batch that removed and failed nothing does not: that is exactly when `touched` hands the splice no paths at all, so the arena — and every id in it — is untouched, and taking the user out of their directory would be a lie about what happened. The mock cannot stage the renumbering (its fixture keeps ids and drops deleted ones), which is why this is pinned by "goes back to the top of the tree" rather than by the shuffle itself.

**The header total is this task's too, and `ScanController` gains `refresh` for it.** Task 11 named the problem and left the choice open; Task 16's e2e asserts the total shrinks, so it has to be made here. `scan_status` already answers with `patched_stats` — bytes, files and folders from the tree the splice installed — so the seam is one command and no new type: `refresh()` re-reads it, applies it only over a terminal state (a running scan's events are fresher than any reply), and deliberately does not bump the generation. Taking the total from the root `NodeView` instead was the other option and is worse: it fixes the bytes and leaves the file and folder counts of a tree that is gone in the same sentence.

**Backspace belongs to the page, and the dialog is modal.** The page's "up one directory" listener is on the window, so an open dialog does not stop it: one Backspace and the Explorer walks up behind the modal, taking the selection the dialog is asking about with it. The listener is suppressed while a deletion is on screen — which includes the preview round trip, when there is no overlay at all, and **not** the banner a refused preview leaves: that is a message, not a dialog, and a page that holds the keyboard until someone finds Dismiss is a page that is stuck.

**A deletion is scoped like the ticks are, and an unanswered one is dropped.** All three phases carry the tree and the directory they were asked in. The rule is `status.phase === 'asking'`, not "not a dialog": a *question* may be dropped, an *answer* may not — once the batch has run, that report is the only thing that will ever say `recorded` or `treeStale`, and this page's own re-anchor moves the scope out from under it every time.

Scoping only the two pre-dialog phases was the first attempt, on the argument that nothing can move the scope under a modal. True of the modal, false of the round trip that creates it: **Rescan sits in the header the whole time "Checking what would be deleted…" is up**, and a scan *start* moves neither the generation nor the directory, so the reply was accepted, the dialog opened, and `scan:done` then renumbered the tree underneath it — a modal nobody asked for, over a tree with nothing ticked, whose button would move 28.7 GB against sizes and verdicts read before the rescan. Without the scope, three more things went wrong the same way: a banner naming rows that are gone survived a navigation and a rescan (holding Backspace with it), the bar narrated "Checking…" over rows nobody had asked about, and the reply opened a modal over them.

The reply also checks the number of the preview it belongs to, and gives that number back only if it is still its own: one flag for two calls lets a reply that came back to a page which had moved on take the place of the question someone asked after it — and then *that* dialog never opens, with nothing on screen to say why. The same scope change frees the flag, because a page that looks idle and ignores the click is worse than one that looks busy.

**The dialog is mounted outside the branch that draws the tree.** A rescan blanks the current node while the new root loads, and a dialog mounted in there vanishes mid-batch and comes back a stranger: new focus, and the mode reset to whatever it opened in. What ends a dialog is the scope rule and `onClose`, not the loading state of something behind it.

**The action bar keeps its place in the layout when it has nothing to say.** Measured at 1280 px: showing it moves everything below by 61.4 px against a row of 34.5 px, so the first tick slides the next row the user is aiming at almost two rows up — on the one screen in this app that deletes things. It is rendered always and `invisible` when empty, which also takes its buttons out of the tab order and (with `aria-hidden`) out of the accessibility tree; they are disabled for the third way in. The banner of a refused preview lives inside it for the same reason: a block of its own shifted the table again, asynchronously, while the pointer was over a row.

**The trade, which is not free.** That band is now on every Explorer screen, ticked or not: 61.4 px of nothing between the treemap and the table, which at the window's 900×600 minimum is the difference between one row above the fold and none. It buys a table that never moves under the pointer while a selection is being made, which is the interaction that ends in a deletion. Taking the bar out of flow — overlaying it, or floating it at the foot of the content — would buy both, at the cost of covering rows and a positioning problem of its own; that is the alternative to revisit if the band grates. Task 16's `explorer.png` will show it as an empty gap, which is the design and not a rendering fault.

**"Nothing to say" is not "nothing ticked".** The table stays live through the round trip, so the ticks can go before the refusal they caused has been read; the bar shows whenever it has a selection *or* an alert. An alert that hides itself, with its own Dismiss disabled inside it, would leave the only account of a refused deletion invisible to everyone — and it can mount already hidden.

**The message wraps.** Truncating it into a `title` read well at 1280 px and showed an ellipsis and nothing else at the window's own minimum width, with the part that distinguishes "inside a folder this app never deletes from" from "needs Full Disk Access" reachable only by hovering — not by keyboard, not by touch, not announced, not selectable for a bug report. It wraps instead, and the bar grows on the path that has already gone wrong.

**The dialog's confirming button is `danger` in Permanent mode** (Task 13 chose `primary` before a danger style existed). The blue "this is the way on" belonged to the Trash, not to the most dangerous control in the app, and the less so under the red entry point that only opened a dialog. `Button` now renders `data-variant`, so *which* variant a control wears is a decision a test can read while what red looks like stays the stylesheet's. One lookup feeds the attribute and the class, and `Button.test.tsx` checks the pair: on its own the attribute pins the variant that was *asked for*, and painting every button blue would leave every attribute truthful and every test green.

**The action bar is a `group`, not a `toolbar`.** The APG toolbar is one tab stop with arrow keys between its controls; these are three ordinary buttons. It is the same argument that left the table a `table` rather than a `grid`, applied to this task's own markup.
...
**What the invalidation cannot fix, and what the mock cannot show.** `invalidateQueries` marks entries stale and goes on serving them, so a directory the user returns to inside the round trip renders ids from the arena the splice replaced. Nothing can be deleted wrongly — a batch carries absolute paths — and today the open report's backdrop covers the window, which is mitigation by accident; `docs/adr/0005-patching-the-scan-tree.md` has the argument. Neither that nor the renumbering itself can be staged against the mock: `fixtureNodes` is flattened once and a batch only adds to a `removed` set, so the tests here pin the policy ("a batch that patched anything navigates to `ROOT_ID`") and never the hazard it exists for. Making the fixture re-flatten a `let liveNodes` after a patching batch is about fifteen lines and would let a test watch the Explorer refuse to render another directory's rows — **Task 16's**, together with `wrapInvoke` in `src/test/invoke.ts`, which drops `args` and so keeps a test from asserting that the first `tree_node` after a batch carries `id: 0`.

**The e2e selectors move here, not in Task 16.** Task 12's hand-off named `ExplorerPage.test.tsx` only, but rendering the checkbox column breaks `e2e/explorer.spec.ts` the same way, and `just e2e` is part of `just ci` — so `NAME_SPAN` becomes `td:nth-child(2) …` and `delta` becomes `.nth(4)` in this task. Task 16 writes new specs against a table that is already right.
...
**That screenshot is carrying a smaller gap than it was, so review it rather than glance at it.** Task 13 left a mutation survivor here — dropping the confirm button's variant lost the emphasis on the most dangerous control on the screen and nothing failed, because asserting a Tailwind class pins the stylesheet rather than the behaviour. Task 14 took half of it back: `Button` renders `data-variant`, and which variant each button wears is now pinned by a test (`danger` for a permanent deletion, `primary` for the Trash, `danger` for the bar's entry point). What no test can still see is what those variants *look like* — that red reads as red, and that the two reds beside each other are not a wall. Frame the shot with the dialog in Permanent mode over the action bar, where all three meet.
...
**The empty band above the table is deliberate.** The action bar holds its place in the layout whether or not anything is ticked (Task 14 records why), so `explorer.png` shows 61.4 px of nothing between the treemap and the table. Do not "fix" it in the screenshot, and say what it is in the PR body.
