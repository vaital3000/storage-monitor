...
**A deletion that has not become a dialog is scoped like the ticks are.** `previewing` and `previewFailed` carry the tree and the directory they were started in and are dropped when either moves; `confirming` carries neither, because nothing can move the scope under a modal and its report has to survive the one thing that does — this page sending itself back to the root. Without that scope three things went wrong at once, and all three are the same mistake: a banner naming rows that are gone survived a navigation and a rescan (holding Backspace with it), the bar narrated "Checking what would be deleted…" over rows nobody had asked about, and the reply opened a modal over them. The reply checks a token that the same scope change bumps, and the guard is freed there too — a page that looks idle and ignores the click is worse than one that looks busy.
