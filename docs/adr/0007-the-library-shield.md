...
Meanwhile the folder is where the space is. Measured on the maintainer's Mac:
**89 top-level entries**, about 70 GB, of which roughly 30 GB is the kind of thing
a disk tool exists to find — `Caches` 16 GB, `Android` 7.4 GB, `Developer` 3.9 GB
(Xcode's `DerivedData`), `Logs` 1.7 GB, `pnpm` 846 MB. None of it was reachable:
...
subfolders, ages badly in exactly this folder. With 89 entries and third-party
...
would be telling the user to stop in front of the 30 GB they came for.

## Consequences

**The unlock is the whole point, and it is one line of it.** Moving `~/Library`
alone from `denied` to `shielded` is what makes `Caches`, `Android`, `Developer`,
`Logs` and `pnpm` ordinary deletable rows. The three shielded folders under it are
the same trade one level down: 50 of the 70 GB lives in them, they are refused as a
single tick because losing all of an application's data at once is expensive, and
they are cleaned the way anyone would actually clean them — one application at a
time.
...
narrower denylist.** When macOS or an installer adds the ninetieth folder to
