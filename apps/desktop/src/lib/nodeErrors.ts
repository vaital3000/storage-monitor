...
// directory it did not (fully) read. Two of them are not failures at all, and one means
// the directory was listed except for a few entries.
...
/** `3 entries could not be read`: the directory was listed, some of its entries were not. */
const PARTIAL_READ_PATTERN = /^\d+ entr(y|ies) could not be read$/;

export interface NodeErrorMark {
  /**
   * `info`: nothing is wrong, the scanner chose not to read it. `partial`: read, except
   * for some entries. `lock`: it could not be read at all.
   */
  kind: 'info' | 'partial' | 'lock';
...
export function describeNodeError(error: string): NodeErrorMark {
  switch (error) {
    ...
    default:
      return { kind: PARTIAL_READ_PATTERN.test(error) ? 'partial' : 'lock', title: error };
  }
}
